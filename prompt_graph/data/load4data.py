...
from .tools import load_data_new,get_idx_info,make_longtailed_data_remove,refine_label_order,split_manual_lt
def shuffle_together(index_list, label_list):
    combined = list(zip(index_list, label_list))
    random.shuffle(combined)
    index_list[:], label_list[:] = zip(*combined)
    return index_list, label_list


def node_sample_and_save(data, folder, num_classes):
    train_idx = data.train_mask
    tensor = torch.tensor(train_idx)
    train_idx = torch.nonzero(tensor, as_tuple=True)[0]
    train_labels = data.y[data.train_mask]

    val_idx = data.val_mask
    tensor = torch.tensor(val_idx)
    val_idx = torch.nonzero(tensor, as_tuple=True)[0]
    val_labels = data.y[data.val_mask]

    test_idx = data.test_mask
    tensor = torch.tensor(test_idx)
    test_idx = torch.nonzero(tensor, as_tuple=True)[0]
...
def load4node(dataname):
    ...
    if dataname in ['PubMed', 'CiteSeer', 'Cora']:
        dataset = Planetoid(root='data/Planetoid', name=dataname, transform=NormalizeFeatures(),split='full')
        data = dataset[0]
        input_dim = dataset.num_features
        out_dim = dataset.num_classes
        ...
        idx_info = get_idx_info(data.y, out_dim, data_train_mask)
        y = data.y.cpu()

        class_num_list, data_train_mask, idx_info, train_node_mask, train_edge_mask = make_longtailed_data_remove(
            data.edge_index, data.y, n_data, out_dim, 100, data_train_mask.clone())
        edge = data.edge_index[:, train_edge_mask]
        labels, og_to_new = refine_label_order(y)
        if dataname == 'Cora':  # change label 1<->2 in order to maintain descending order
            labels[labels == 1] = 100
            labels[labels == 2] = 1
            labels[labels == 100] = 2

        elif dataname == 'CiteSeer':  # change label 0<->1 in order to maintain descending order
            labels[labels == 0] = 100
            labels[labels == 1] = 0
            labels[labels == 100] = 1
        ...
        idx_train, idx_val, idx_test, class_num_mat = split_manual_lt(labels, idx_train, idx_val, idx_test)
        samples_per_label = torch.tensor(class_num_mat[:, 0])

        train_mask = np.zeros(total_nodes, dtype=np.bool_)
        val_mask = np.zeros(total_nodes, dtype=np.bool_)
        test_mask = np.zeros(total_nodes, dtype=np.bool_)
