...
class BaseTask:
    def __init__(self, pre_train_model_path='None', gnn_type='TransformerConv',
                 hid_dim = 128, num_layer = 2, dataset_name='Cora', prompt_type='None', epochs=100, shot_num=10, device : int = 5, lr =0.0001, wd = 5e-4,
    ...
    def initialize_optimizer(self):
        if self.prompt_type == 'None':
            ...
            #     model_param_group = []
            #     model_param_group.append({"params": self.prompt.parameters()})
            #     model_param_group.append({"params": filter(lambda p: p.requires_grad, self.gnn.parameters())})
            #     self.pg_opi = optim.Adam(model_param_group, lr=self.lr, weight_decay=self.wd)
    ...
    def initialize_prompt(self):
        ...
        #     self.prompt = DiffPoolPrompt(self.input_dim, num_clusters=5 ).to(self.device)
            ...
            self.head_prompt = Gprompt(self.hid_dim).to(self.device)
            self.body_prompt = Gprompt(self.hid_dim).to(self.device)
            self.tail_prompt = Gprompt(self.hid_dim).to(self.device)
        elif self.prompt_type == 'MultiGprompt':
            nonlinearity = 'prelu'
    ...
    def initialize_gnn(self):
        if self.gnn_type == 'GAT':
            ...
            self.gnn_head = GCN(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer)
            self.gnn_body = GCN(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer)
            self.gnn_tail = GCN(input_dim=self.input_dim, hid_dim=self.hid_dim, num_layer=self.num_layer)
